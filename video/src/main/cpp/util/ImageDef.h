...
static const char *TAG = "ImageDefLog";
...
public:
    static void AllocNativeImage(NativeImage *pImage){
        ...
        switch (pImage->format) {
            ...
            case IMAGE_FORMAT_I420:
                ...
                pImage->pLineSize[2] = pImage->height / 2;
                break;

            default:
                LOGCATE(TAG, "NativeImageUtil::AllocNativeImage do not support the format. Format = %d", pImage->format);
                break;
        }
    }
    ...
    static void CopyNativeImage(NativeImage *pSrcImg, NativeImage *pDstImg) {
        LOGCATE(TAG, "NativeImageUtil::CopyNativeImage src[w,h,format]=[%d, %d, %d], dst[w,h,format]=[%d, %d, %d]", pSrcImg->width, pSrcImg->height, pSrcImg->format, pDstImg->width, pDstImg->height, pDstImg->format);
        LOGCATE(TAG, "NativeImageUtil::CopyNativeImage src[line0,line1,line2]=[%d, %d, %d], dst[line0,line1,line2]=[%d, %d, %d]", pSrcImg->pLineSize[0], pSrcImg->pLineSize[1], pSrcImg->pLineSize[2], pDstImg->pLineSize[0], pDstImg->pLineSize[1], pDstImg->pLineSize[2]);
        ...
        if (pSrcImg->format != pDstImg->format
            ...
            LOGCATE(TAG, "NativeImageUtil::CopyNativeImage invalid params.");
            return;
        }
        ...
        switch (pSrcImg->format) {
            ...
            case IMAGE_FORMAT_RGBA:
                ...
                break;
            default:
                LOGCATE(TAG, "NativeImageUtil::CopyNativeImage do not support the format. Format = %d", pSrcImg->format);
                break;
        }
    }
};
